...
| [upper bound] HPPC-RLS | 0 | 50.02 | 71.0 % (+11.7) | **33.69** | 55.1 % (+4.4) |
...
of 33.69 mV is the best in the table. On discharge it beats A3 by only +0.8 %p.

That is, **even if a vehicle could afford periodic HPPC, deployment performance
would barely improve.** The voltage gain of §32.4 (58.76 -> 50.02 mV) disappears
...
### 38.1 The HPPC-RLS baseline was reading its own answer

`sop_trim_dataset.py` pairs each measured pulse with twelve preceding drive
blocks, so **one physical pulse is twelve rows carrying the same target** —
896 pulses in 10752 rows for CC, and no key holding two different `Y`.
`sop_rls.rls_cell` walked those rows one at a time: predict, update, next row.
Rows 2 to 12 of a pulse were therefore predicted by a `theta` already updated
up to twenty-two times on that pulse's own answer.

Measured with a probe that perturbs one pulse's `Y` by 50 mV and asks whether
that pulse's own predictions move: **they moved on 6 of 6 probed pulses, by up
to 4.81 mV.** After the fix they do not move at all.

The unit is now the physical pulse, keyed `(cycle, SOC, rank)`. Pulses are
ordered by `(cycle, -SOC)` — within a cycle the HPPC steps SOC down
monotonically, which the data shows — and pulses sharing `(cycle, SOC)` are
treated as **simultaneous**, because the order of the two current ranks at one
SOC is recorded nowhere and guessing it would invent causality the files do not
have. Each pulse target updates once.

**What it cost, and why that number is not reassuring.**

| | before | after |
|---|---:|---:|
| voltage RMSE, discharge | 49.89 mV | 50.02 |
| voltage RMSE, charge | 33.59 mV | 33.69 |
| ladder usable, discharge τ=10 | 71.1 % | 71.0 |
| ladder usable, charge τ=10 | 55.2 % | 55.1 |

Small — but that is a property of the shipped setting, not of the defect.
`sop_baseline_fill.py` runs at **ff = 1.0**, where `P` shrinks monotonically
and a twelfth update on the same target adds almost nothing. At **ff = 0.999**
the same defect is worth 32.06 → 44.14 mV pooled, a 38 % change. A baseline
that reads its own answer is not a baseline whatever the forgetting factor
happens to be, so it is fixed rather than argued away.

**Scope.** HPPC-RLS is not one of the six methods in the main comparison; it
appears as the explicitly labelled `[upper bound] HPPC-RLS` ladder row and in
`voltage.csv`. Four stored checks moved and no other table did:
`runs_trim_direct` and `runs_trim_shrink`, regenerated by the same script in
the same run, came back byte-identical, and `shrink` **is** one of the six.
